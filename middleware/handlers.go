...
import (
	...
	"fmt"
	"go-postgres-yt/models" // models package where Stock schema is defined
	"log"
	...
	// "_" indicates its not used in the code directly, imported only for registring pg driver
)
...
func createConnection() *sql.DB {
	...
	// Open the connection
	db, err := sql.Open("postgres", os.Getenv("POSTGRES_URL") /* +" sslmode=require" */)
	if err != nil {
		panic(err)
	}
...
func DeleteStock(w http.ResponseWriter, r *http.Request) {
	...
	// get the stockid from the request params, key is "id"
	params := mux.Vars(r) // gets params from request using mux
...
// -------------------- handler functions (DB interactions) ----------------

// insert one stock in the DB
func insertStock(stock models.Stock) int64 {

	db := createConnection()
	defer db.Close() // to close the db at the end
